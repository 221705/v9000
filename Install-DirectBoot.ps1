...
$srcDir = $PSScriptRoot
# $ToolHashes comes from Bcd.Common.ps1 - single source of truth, shared with
...
foreach ($name in $ToolHashes.Keys) {
    ...
    if ($h -ne $ToolHashes[$name]) { Fail "Hash mismatch for '$name' - file corrupted or modified." }
}
...
if (-not $gDirect) {
    $gDirect = New-BcdFirmwareEntry $descDirect
...
if (-not $gShell) {
    $gShell = New-BcdFirmwareEntry $descShell
...
# ---- 9. Verify ----
$verOrder = Get-BcdFwbootmgrDisplayOrder
Info ''
